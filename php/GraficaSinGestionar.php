...
//Consulta Todos Casos Asignados
$ConsultaSQL2 = "SELECT PKPENCAL_NCODIGO FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_CALL WHERE (PENCAL_COBSERVACIONES IS NULL OR PENCAL_COBSERVACIONES= '') AND (PENCAL_CESTADO_FINAL2 IS NULL OR PENCAL_CESTADO_FINAL2= '') AND FKPENCAL_NPKPER_NCODIGO IS NOT NULL AND PENCAL_CFECHA_REGISTRO BETWEEN '" . $FechaInicial . "' AND '" . $FechaFinal . "' AND PENCAL_CESTADO = 'Activo' GROUP BY FKPENCAL_NPKCLI_NCODIGO;";
if ($ResultadoSQL2 = $ConexionSQL->query($ConsultaSQL2)) {
    $CantidadResultados2 = $ResultadoSQL2->num_rows;
    if ($CantidadResultados2 > 0) {
        $CasosAsignados = $CantidadResultados2;
...
//Consulta Casos Sin Asignar
$ConsultaSQL3 = "SELECT PKPENCAL_NCODIGO FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_CALL WHERE (PENCAL_COBSERVACIONES IS NULL OR PENCAL_COBSERVACIONES= '') AND (PENCAL_CESTADO_FINAL2 IS NULL OR PENCAL_CESTADO_FINAL2= '') AND FKPENCAL_NPKPER_NCODIGO IS NULL AND PENCAL_CFECHA_REGISTRO BETWEEN '" . $FechaInicial . "' AND '" . $FechaFinal . "' AND PENCAL_CESTADO = 'Activo' GROUP BY FKPENCAL_NPKCLI_NCODIGO;";
if ($ResultadoSQL3 = $ConexionSQL->query($ConsultaSQL3)) {
    $CantidadResultados3 = $ResultadoSQL3->num_rows;
    if ($CantidadResultados3 > 0) {
        $CasosSinAsignar = $CantidadResultados3;
...
/*
print_r($CasosAsignados);
print_r($CasosSinAsignar);
exit;*/
...
<head>
    ...
    <title>Reporte Casos Sin Gestionar</title>
...
<body>
    ...
    <div>
        <form method="POST" action="GraficaSinGestionar.php" enctype="multipart/form-data">
    ...
    <script>
        ...
        const myChart = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: ['Casos Asignados', 'Casos Sin Asignar'],
                datasets: [{
                    label: 'Numero De Casos',
                    data: [<?php echo $CasosAsignados; ?>, <?php echo $CasosSinAsignar; ?>],
                    backgroundColor: [
                        'rgba(0, 128, 0, 0.8)',
                        'rgba(255, 69, 0, 0.8)',
                        'rgba(173, 255, 47, 0.8)',
