...
$Datos3 = array();
$ConsultaSQL = "SELECT PKPENAFL_NCODIGO, CLI_CDOCUMENTO, FKPENAFL_NPKPENCAL_NCODIGO, CONCAT (CLI_CNOMBRE,' ',CLI_CNOMBRE2,' ',CLI_CAPELLIDO,' ',CLI_CAPELLIDO2) AS NOMBRE_CLIENTE, PENAFL_CFONDO_NUEVO, PENALF_CFECHA_REGISTRO, DETCLI_CDETALLE FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_AFILIACION, u632406828_dbp_crmfuturus.TBL_RCLIENTE, u632406828_dbp_crmfuturus.TBL_RDETALLE_CLIENTE WHERE PKCLI_NCODIGO = FKPENAFL_NPKPENCAL_NCODIGO AND PENALF_CESTADO= 'Activo' AND CLI_CESTADO= 'Activo' AND (PENAFL_CESTADO_FINAL2_AGENDA = 'Envio De Afiliacion Exitoso' OR PENAFL_CESTADO_FINAL2_AGENDA= 'Afiliacion Exitosa') AND (PENAFL_CESTADO_FINAL2 IS NULL OR PENAFL_CESTADO_FINAL2 = '') group by CLI_CDOCUMENTO;";
if ($ResultadoSQL = $ConexionSQL->query($ConsultaSQL)) {
    $CantidadResultados = $ResultadoSQL->num_rows;
    if ($CantidadResultados > 0) {
        while ($FilaResultado = $ResultadoSQL->fetch_assoc()) {
            ...
            $FECHA = $FilaResultado['PENALF_CFECHA_REGISTRO'];
...
$FechaFuturus = array();
$FechaActual  = array();
$ConsultaSQL = "SELECT PENALF_CFECHA_REGISTRO FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_AFILIACION WHERE (PENAFL_CESTADO_FINAL2_AGENDA = 'Envio De Afiliacion Exitoso' OR PENAFL_CESTADO_FINAL2_AGENDA='Afiliacion Exitosa') AND (PENAFL_CESTADO_FINAL IS NULL OR PENAFL_CESTADO_FINAL ='' ) AND (PENAFL_CESTADO_FINAL2 IS NULL OR PENAFL_CESTADO_FINAL2 = '') AND PENALF_CESTADO = 'Activo';";
if ($ResultadoSQL = $ConexionSQL->query($ConsultaSQL)) {
    $CantidadResultados = $ResultadoSQL->num_rows;
    if ($CantidadResultados > 0) {
        while ($FilaResultado = $ResultadoSQL->fetch_assoc()) {
            $FechaLegalizacion = $FilaResultado['PENALF_CFECHA_REGISTRO'];
            ...
            /*print_r($FechaLegalizacion);
            print_r($FechaFuturus);
            print_r($FechaActual);
            exit;*/
        }
    } else {
                        ...
                        <div class="row">
                            <div class="col-sm-3 col-md-3 col-lg-3 col-xl-3 d-none d-sm-none d-md-none d-lg-block">
                                <div class="navbar-brand" style="margin-top: 1%;">
                                    <a href="EnviosAfiliacion.php"><img width="50%" src="../images/FUTURUS.png" alt="Lucid Logo" class="img-responsive logo"></a>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-3 col-xl-3 d-block d-sm-block d-md-block d-lg-none" style="text-align: center;">
                                <div class="navbar-brand" style="margin-top: 1%;">
                                    <a href="EnviosAfiliacion.php"><img width="60%" src="../images/FUTURUS.png" alt="Lucid Logo" class="img-responsive logo"></a>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    ...
                                    for ($i = 0; $i < count($Datos3); $i++) {
                                        ...
                                        for ($b = 0; $b < count($Datos3[$i]); $b++) {
                                            if ($FechaFuturus[0][0] <= $FechaActual[0][0]) {
                                                if ($b == 4) {
