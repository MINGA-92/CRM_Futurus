...
for ($i = 0; $i < count($CantidadCasos); $i++) {
    ...
    $ConsultaSql = "SELECT FKPENAFL_NPKPENCAL_NCODIGO, PENAFL_CFECHA_AGENDAMIENTO, PENAFL_CFONDO_ANTERIOR, PENAFL_CFONDO_NUEVO, PENAFL_CSALARIO_ACTUAL FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_AFILIACION WHERE PKPENAFL_NCODIGO = " . $CodigoCaso . ";";
    if ($ResultadoSql = $ConexionSQL->query($ConsultaSql)) {
        ...
        if ($ResultadoSql = $ConexionSQL->query($ConsultaSql)) {
            ...
            if ($CantidadResultados > 0) {
                ...
                $PENAFL_CSALARIO_ACTUAL = $FilaResultado['PENAFL_CSALARIO_ACTUAL'];
                ...
                if ($ResultadoSql = $ConexionSQL->query($UpdateSql)) {
                    $InsercionSql = "INSERT INTO u632406828_dbp_crmfuturus.TBL_RPENSIONES_AFILIACION (FKPENAFL_NPKPENCAL_NCODIGO, FKPENAFL_NPKPER_NCODIGO, PENAFL_CFECHA_AGENDAMIENTO, PENAFL_CFONDO_ANTERIOR, PENAFL_CFONDO_NUEVO, PENAFL_CESTADO_FINAL_AGENDA, PENAFL_CSALARIO_ACTUAL, PENALF_CDETALLE_REGISTRO, PENALF_CESTADO) VALUES ('" . $FKPENAFL_NPKPENCAL_NCODIGO . "', '" . $id . "', '" . $PENAFL_CFECHA_AGENDAMIENTO . "', '" . $PENAFL_CFONDO_ANTERIOR . "', '". $PENAFL_CFONDO_NUEVO ."', 'Asignado', '" . $PENAFL_CSALARIO_ACTUAL ."', '" . $RegistradoPor. "', '" . $Estado1 . "');";
