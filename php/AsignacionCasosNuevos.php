...
$Datos3 = array();
$ConsultaSQL = "SELECT PKPENAFL_NCODIGO, CLI_CDOCUMENTO, FKPENAFL_NPKPENCAL_NCODIGO, CONCAT (CLI_CNOMBRE,' ',CLI_CNOMBRE2,' ',CLI_CAPELLIDO,' ',CLI_CAPELLIDO2) AS NOMBRE_CLIENTE, PENCAL_CFONDO_NUEVO, PENAFL_CFECHA_AGENDAMIENTO FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_AFILIACION, u632406828_dbp_crmfuturus.TBL_RCLIENTE, u632406828_dbp_crmfuturus.TBL_RPENSIONES_CALL WHERE PKCLI_NCODIGO = FKPENAFL_NPKPENCAL_NCODIGO AND PENALF_CESTADO= 'Activo' AND CLI_CESTADO= 'Activo' AND FKPENAFL_NPKPER_NCODIGO IS NULL GROUP BY PKPENAFL_NCODIGO;";
if ($ResultadoSQL = $ConexionSQL->query($ConsultaSQL)) {
    $CantidadResultados = $ResultadoSQL->num_rows;
    if ($CantidadResultados > 0) {
        while ($FilaResultado = $ResultadoSQL->fetch_assoc()) {
            ...
            $CodigoCaso = $FilaResultado['PKPENAFL_NCODIGO'];
            
            array_push($Datos3, array("0" => $Documento, "1" => $nombre, "2" => $FONDO, "3" => $FECHA, "4" => $CodigoCaso));
                                    ...
                                    <?php
                                        ...
                                        for ($i = 0; $i < count($Datos3); $i++) {
                                            ...
                                            for ($b = 0; $b < count($Datos3[$i]); $b++) {
                                                ...
                                                if ($b > 3) {
                                                    echo '<td style="text-align: center;"><label><input type="checkbox" class="btn-check" name="checks[]" id="btn-check" autocomplete="off" value="' . $Datos3[$i][5] . '"/><span></span></label></td>';
                                                } else {
                ...
                $.ajax({
                    ...
                    success: function(php_response){
                        ...
                        if(Respuesta == "Ok"){
                            ...
                            $("#Loading").hide();
                            $("#tabla2_wrapper").show();
                            window.location = "AsignacionCasosNuevos.php";
