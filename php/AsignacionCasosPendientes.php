...
$Datos3 = array();
$ConsultaSQL = "SELECT PKPENAFL_NCODIGO, CRE_CUSUARIO AS NOMBRE_USUARIO ,CLI_CDOCUMENTO, CONCAT (CLI_CNOMBRE,' ',CLI_CNOMBRE2,' ',CLI_CAPELLIDO,' ',CLI_CAPELLIDO2) AS NOMBRE_CLIENTE, PENCAL_CFONDO_NUEVO, PENAFL_CFECHA_AGENDAMIENTO, DETCLI_CDETALLE FROM TBL_RPENSIONES_AFILIACION, TBL_RCLIENTE, TBL_RDETALLE_CLIENTE, TBL_RPENSIONES_CALL, TBL_RCREDENCIAL WHERE PKCRE_NCODIGO=FKPENAFL_NPKPER_NCODIGO and FKPENAFL_NPKPENCAL_NCODIGO = PKCLI_NCODIGO AND FKPENAFL_NPKPENCAL_NCODIGO = PKPENCAL_NCODIGO AND FKPENAFL_NPKPENCAL_NCODIGO = FKDETCLI_NCLI_NCODIGO AND DETCLI_CCONSULTA='DireccionDomicilio' AND PENALF_CESTADO= 'Activo' AND CLI_CESTADO= 'Activo' AND DETCLI_CESTADO= 'Activo' AND PENAFL_CESTADO_FINAL_AGENDA IN('Pendiente', 'Asignado') group by PKPENAFL_NCODIGO;";
if ($ResultadoSQL = $ConexionSQL->query($ConsultaSQL)) {
    $CantidadResultados = $ResultadoSQL->num_rows;
    if ($CantidadResultados > 0) {
        while ($FilaResultado = $ResultadoSQL->fetch_assoc()) {
                ...
                <div class="row">
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12" style="text-align: center !important;">
                        <div class="navbar-brand d-none d-sm-block d-md-block" style="margin-bottom: 3%;">
                            <h6>ASIGNACION DE CASOS PENDIENTES</h6>
                        </div>
    ...
    <script>
        ...
        $("body").on('click', '.AsignarCaso', function(){
            ...
            if (casos == ""){
                ...
                $.ajax({
                    url: "GuardarCasosPendientes.php",
                    type: 'POST',
                    dataType: "json",
                    cache: false,
                    processData: false,
                    ...
                    success: function(php_response){
                        ...
                        if(Respuesta == "Ok"){
                            ...
                            $("#Loading").hide();
                            $("#tabla2_wrapper").show();
                            window.location = "AsignacionCasosPendientes.php";
