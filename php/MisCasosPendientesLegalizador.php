...
$Datos3 = array();
$ConsultaSQL = "SELECT PKPENAFL_NCODIGO, CLI_CDOCUMENTO, FKPENAFL_NPKPENCAL_NCODIGO, CONCAT (CLI_CNOMBRE,' ',CLI_CNOMBRE2,' ',CLI_CAPELLIDO,' ',CLI_CAPELLIDO2) AS NOMBRE_CLIENTE, PENCAL_CFONDO_NUEVO, PENAFL_CFECHA_AGENDAMIENTO, DETCLI_CDETALLE FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_AFILIACION, u632406828_dbp_crmfuturus.TBL_RCLIENTE, u632406828_dbp_crmfuturus.TBL_RDETALLE_CLIENTE, u632406828_dbp_crmfuturus.TBL_RPENSIONES_CALL WHERE PKCLI_NCODIGO = FKPENAFL_NPKPENCAL_NCODIGO AND PKPENCAL_NCODIGO = FKPENAFL_NPKPENCAL_NCODIGO AND FKPENAFL_NPKPENCAL_NCODIGO = FKDETCLI_NCLI_NCODIGO AND DETCLI_CCONSULTA='DireccionDomicilio' AND PENALF_CESTADO= 'Activo' AND CLI_CESTADO= 'Activo' AND DETCLI_CESTADO= 'Activo' AND (PENAFL_CESTADO_FINAL_LEGALIZACION= 'Pendiente' OR PENAFL_CESTADO_FINAL2_LEGALIZACION = 'Documentos Pendientes') AND FKPENAFL_NPKPER_NCODIGO_LEGALIZADOR = '".$AGENTE."' group by PKPENAFL_NCODIGO;";
...
if ($ResultadoSQL = $ConexionSQL->query($ConsultaSQL)) {
    ...
    if ($CantidadResultados > 0) {
        while ($FilaResultado = $ResultadoSQL->fetch_assoc()) {
            ...
            array_push($Datos3, array("0" => $Documento, "1" => $nombre, "2" => $FONDO, "3" => $FECHA, "4" => $Direccion, "5"  => $FKPENAFL_NPKPENCAL_NCODIGO));
        }
    } else {
        //Sin Resultados
    }
...
<head>
    <title> Mis Casos Pendientes :: Futurus </title>
    <meta charset="utf-8">
                ...
                <div class="row">
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12" style="text-align: center !important;">
                        <div class="navbar-brand d-none d-sm-block d-md-block" style="margin-bottom: 3%;">
                            <h4>CASOS PENDIENTES PARA LEGALIZAR</h4>
                        </div>
                            ...
                            <table id="tabla" class="table table-bordered table-striped table-responsive-sm-md-lg-xl dt-responsive table-hover dataTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: center;">Dirrecion</th>
                                    ...
                                    <?php
                                        ...
                                        for ($i = 0; $i < count($Datos3); $i++) {
                                            ...
                                            for ($b = 0; $b < count($Datos3[$i]); $b++) {
                                                ...
                                                if($b == 5){
                                                    echo '<td style="text-align: center;"><label><a onclick="enviarInformacionLegalizador(' . $Datos3[$i][$b] .');"id="SeleccionarCaso" class="btn btn-futurus-r " data-target="1" value=""><i class="fa icon-eye"></i></a><span></span></label></td>';
                                                }else{
