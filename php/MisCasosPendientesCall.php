...
$Datos2 = array();
$ConsultaSQL = "SELECT PKPENCAL_NCODIGO, CLI_CDOCUMENTO AS DOCUMENTO, CONCAT (CLI_CNOMBRE,' ',CLI_CNOMBRE2,' ',CLI_CAPELLIDO,' ',CLI_CAPELLIDO2) AS NOMBRE_CLIENTE, PENCAL_CFONDO_NUEVO AS FONDO, PENCAL_CESTADO_FINAL2 AS EstadoCliente, PENCAL_CFECHA_REGISTRO AS UltimaGestion FROM u632406828_dbp_crmfuturus.TBL_RPENSIONES_CALL, u632406828_dbp_crmfuturus.TBL_RCLIENTE, u632406828_dbp_crmfuturus.TBL_RDETALLE_CLIENTE WHERE PKCLI_NCODIGO = FKDETCLI_NCLI_NCODIGO AND FKPENCAL_NPKCLI_NCODIGO = PKCLI_NCODIGO AND DETCLI_CCONSULTA = 'FondoPensionCliente' AND FKPENCAL_NPKPER_NCODIGO = " . $AGENTE . " AND (PENCAL_CESTADO_FINAL2 = 'Cliente Indeciso' OR PENCAL_CESTADO_FINAL2 = 'Rellamada') AND PENCAL_CESTADO = 'Activo';";
...
if ($ResultadoSQL = $ConexionSQL->query($ConsultaSQL)) {
    ...
    if ($CantidadResultados > 0) {
        while ($FilaResultado = $ResultadoSQL->fetch_assoc()) {
            ...
            array_push($Datos2, array("0" => $CLI_CDOCUMENTO, "1" => $CLI_CNOMBRE, "2" => $DETCLI_CDETALLE, "3" => $EstadoCliente, "4" => $UltimaGestion, "5" => $PKPENCAL_NCODIGO));
        }
    } else {
        //Sin Resultados
    }
                                    ...
                                    <?php
                                        for ($i = 0; $i < count($Datos2); $i++) {
                                            ...
                                            for ($b = 0; $b < count($Datos2[$i]); $b++) {
                                                if($b == 5){
                                                    echo '<td style="text-align: center;"><label><a onclick="enviarInformacion(' . $Datos2[$i][$b] .');" id="SeleccionarCaso" class="btn btn-futurus-r " data-target="1" value=""><i class="fa icon-eye"></i></a><span></span></label></td>';
                                                }else{
